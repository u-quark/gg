{ sources, haskellLib, callCabal2nix }:
...
self: super:
{
  MissingH = modifyPkg (callCabal2nix "MissingH" sources.MissingH {
    hslogger = self.hslogger;
    regex-compat = self.regex-compat;
  });
  vty = modifyPkg (callCabal2nix "vty" sources.vty {
    microlens-th = self.microlens-th;
  });
  brick = modifyPkg (callCabal2nix "brick" sources.brick {
    vty = self.vty;
    microlens-th = self.microlens-th;
  });
